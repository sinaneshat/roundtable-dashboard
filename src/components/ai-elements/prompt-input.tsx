    ...
    >
      <div className="space-y-2 py-1" ref={contentRef}>
        <div className="flex flex-wrap gap-2">
          {attachments.files
            .filter((f: FileUIPart & { id: string }) => !(f.mediaType?.startsWith('image/') && f.url))
            .map((file: FileUIPart & { id: string }) => (
...
export function PromptInput({
  ...
  const ctx = useMemo<AttachmentsContext>(
    () => ({
      files: files.map((item: FileUIPart & { id: string }) => ({ ...item, id: item.id })),
      add,
      remove,
      clear,
      openFileDialog,
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    Promise.all(
      files.map(async ({ id, ...item }: FileUIPart & { id: string }) => {
...
export function PromptInputTextarea({
  ...
  return (
    <Textarea
      className={cn(
        'w-full resize-none border-none p-3 shadow-none outline-none ring-0',
