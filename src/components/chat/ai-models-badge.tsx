...
export function AIModelsBadge({
  ...
  const sizeConfig = {
    sm: { avatar: 'size-7', badge: 'h-5 min-w-[20px] text-[10px] px-1.5' },
    md: { avatar: 'size-8', badge: 'h-6 min-w-[24px] text-xs px-2' },
    lg: { avatar: 'size-9', badge: 'h-7 min-w-[28px] text-sm px-2' },
  };
      ...
      <div className="flex -space-x-3">
        {displayedModels.map((model, idx) => {
          ...
          return (
            <Avatar
              key={model.modelId}
              className={cn(config.avatar, 'ring-2 ring-black bg-black relative')}
              style={{ zIndex: displayedModels.length - idx }}
            >
              <AvatarImage
