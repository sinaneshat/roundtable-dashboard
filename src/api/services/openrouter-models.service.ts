...
class OpenRouterModelsService {
  ...
  async fetchAllModels(): Promise<BaseModelResponse[]> {
    ...
    // ✅ FILTER: Exclude single-turn only models (e.g., Morph)
    // Some models are specialized for single-turn interactions and don't support multi-turn conversations
    // These models will reject requests with multiple messages in the conversation history
    const multiTurnModels = compatibleModels.filter(model => !this.isSingleTurnOnlyModel(model));

    // Enhance models with computed fields
    const enhancedModels = multiTurnModels.map(model => this.enhanceModel(model));
  ...
  private requiresSpecialDataPolicy(model: RawOpenRouterModel): boolean {
    ...
    // ═══════════════════════════════════════════════════════════════
    // ✅ FALLBACK PROTECTION: Specific models that bypass provider filtering
    // This list catches edge cases where models slip through other filters
    const dataPolicyBlocklist = [
      'microsoft/mai-ds-r1', // Requires "Paid model training" opt-in
      'tngtech/deepseek-r1t-chimera', // Merge model requiring data policy opt-in
      'tngtech/deepseek-r1t2-chimera', // Merge model requiring data policy opt-in
      'alpindale/goliath-120b', // Merge model: "combining two fine-tuned Llama 70B"
      'undi95/remm-slerp-l2-13b', // Merge model with #merge tag
  ...
  private isFineTunedOrTrainingModel(model: RawOpenRouterModel): boolean {
    ...
    //
    // ✅ COMPREHENSIVE LIST: Based on analysis of OpenRouter API (Jan 2025)
    // All providers below create derivative works (fine-tunes, merges, variants)
    // that require special data policy opt-ins not available to standard users
    const fineTuningProviders = [
      'nousresearch', // Nous Research - fine-tuning organization (8 models)
      'arliai', // ArliAI - fine-tuning organization (2 models)
      'cognitivecomputations', // Cognitive Computations - fine-tuning org (3 models)
      'gryphe', // Gryphe - fine-tuning organization, creates #merge models (1 model)
      'sao10k', // sao10k - creates "strategic merge" models (5 models)
      'sophosympatheia', // Sophosympatheia - fine-tuning organization (historical)
      'neversleep', // NeverSleep - creates #merge and #uncensored models (2 models)
      'fimbulvetr', // Fimbulvetr - fine-tuning organization (historical)
      'tngtech', // TNG Technology Consulting - creates chimera merge models (4 models)
  ...
  /**
   * ✅ FILTER: Exclude single-turn only models
   *
   * **Problem**: Some specialized models (especially code editing models like Morph)
   * only support single-turn interactions. They reject requests with multiple messages
   * in the conversation history, causing "Multi-turn conversations are not supported" errors.
   *
   * **Error Example** (from morph/morph-v3-large):
   * "Multi-turn conversations are not supported" (HTTP 400)
   *
   * **Detection Strategy**:
   * 1. Provider-based filtering: Known single-turn model providers (Morph)
   * 2. Description analysis: Models describing themselves as "apply models" or "code editing"
   * 3. Model ID patterns: Models with specific single-turn identifiers
   *
   * **Single-Turn Models** (EXCLUDED):
   * - morph/morph-v3-large - Code editing model requiring specific XML format
   * - morph/morph-v3-* - All Morph models are single-turn code editors
   *
   * **Why Exclude**: These models are designed for specialized single-turn tasks (code editing)
   * and don't fit the multi-participant conversation pattern used in this application.
   *
   * @param model - Raw model from OpenRouter API
   * @returns true if model only supports single-turn interactions (should be excluded)
   */
  private isSingleTurnOnlyModel(model: RawOpenRouterModel): boolean {
    const modelId = model.id.toLowerCase();
    const description = model.description?.toLowerCase() || '';
    const provider = modelId.split('/')[0] || '';

    // ═══════════════════════════════════════════════════════════════
    // PROVIDER-BASED FILTERING: Known single-turn only providers
    // ═══════════════════════════════════════════════════════════════
    // Morph: Specialized code editing models that only accept single messages
    // with specific XML format: <instruction>, <code>, <update>
    const singleTurnProviders = [
      'morph', // Morph - code editing models requiring single-turn interactions
    ];

    if (singleTurnProviders.includes(provider)) {
      return true; // Exclude all models from single-turn providers
    }

    // ═══════════════════════════════════════════════════════════════
    // DESCRIPTION-BASED DETECTION: Code editing and apply model patterns
    // ═══════════════════════════════════════════════════════════════
    // Models describing themselves as specialized "apply" or code editing tools
    // typically don't support conversational multi-turn interactions
    const singleTurnIndicators = [
      'apply model', // e.g., "high-accuracy apply model for complex code edits"
      'code editing model', // Explicit code editing specialization
      'single-turn', // Explicit single-turn statement
      'xml format required', // Models requiring specific input formats (not conversational)
    ];

    for (const indicator of singleTurnIndicators) {
      if (description.includes(indicator)) {
        return true; // Exclude single-turn specialized models
      }
    }

    // ═══════════════════════════════════════════════════════════════
    // PASS: Model supports multi-turn conversations
    // ═══════════════════════════════════════════════════════════════
    return false;
  }
