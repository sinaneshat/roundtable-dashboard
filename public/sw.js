...
globalThis.addEventListener('activate', (event) => {
  ...
  event.waitUntil(
    ...
    }).then(() => globalThis.clients.claim()), // Take control of all pages immediately
  );
});
...
globalThis.addEventListener('push', (event) => {
  ...
  try {
    ...
    event.waitUntil(
      globalThis.registration.showNotification(data.title || 'Roundtable', options),
    );
  } catch {
    // Handle non-JSON push data
    const text = event.data.text();
    event.waitUntil(
      globalThis.registration.showNotification('Roundtable', { body: text }),
    );
  }
});
