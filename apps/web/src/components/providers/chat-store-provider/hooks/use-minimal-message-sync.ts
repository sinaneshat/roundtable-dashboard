...
export function useMinimalMessageSync({ chat, store }: UseMinimalMessageSyncParams) {
  ...
  const syncToStore = useCallback(() => {
    ...
    // ✅ DEBUG: Track moderator messages through sync
    const storeMods = currentStoreMessages.filter(m => isModeratorMessage(m));
    const preservedMods = storeOnlyMessages.filter(m => isModeratorMessage(m));
    const mergedMods = mergedMessages.filter(m => isModeratorMessage(m));
    rlog.moderator('msg-sync', `store=${storeMods.length} preserved=${preservedMods.length} merged=${mergedMods.length} chat=${filteredChatMessages.length}`);
