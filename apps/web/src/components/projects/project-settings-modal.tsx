...
import { MemoryDeleteDialog } from '@/components/projects/memory-delete-dialog';
import { ProjectIconBadge, ProjectIconColorPicker } from '@/components/projects/project-icon-color-picker';
import { ProjectMemoryCard } from '@/components/projects/project-memory-card';
...
import { useDownloadUrlQuery, useProjectAttachmentsQuery, useProjectMemoriesQuery } from '@/hooks/queries';
                        ...
                        : memories.length === 0
                          ? (
                              <div className="text-center py-6 border border-dashed border-border/50 rounded-xl">
                                <Icons.brain className="size-5 mx-auto text-muted-foreground/60" />
                              ...
                              <div className="space-y-2">
                                {memories.map(memory => (
                                  <ProjectMemoryCard
                                    key={memory.id}
                                    memory={memory}
                                    onDelete={() => setMemoryToDelete(memory.id)}
                                  />
                                ))}
                              </div>
                            )}
