 ...
 * Runtime access to secrets uses cloudflare:workers env (see ses-service.ts).
 */
export const EMAIL_SERVICE_CONFIG = {
  FROM_ADDRESS: process.env.SES_VERIFIED_EMAIL || 'noreply@roundtable.now',
  FROM_NAME: 'Roundtable',
  REPLY_TO: process.env.SES_REPLY_TO_EMAIL || 'support@roundtable.now',
