...
- `src/components/{domain}/__tests__/` - Component-specific tests
- `src/stores/{domain}/__tests__/` - Store/state management tests
...
**Data Mocking:**
- Mock API responses using Jest's `jest.fn()`
- Create reusable mock data factories in `src/lib/testing/helpers.ts`
...
8. **ESLint compliance** - Follow testing-library, jest, and jest-dom ESLint rules
...
- Global mocks in `jest.setup.ts` (matchMedia, IntersectionObserver, etc.)
